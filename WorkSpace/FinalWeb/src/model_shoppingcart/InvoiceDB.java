...
public class InvoiceDB {
	...
	public boolean addStringDate(int id, int idCuctomer, String ngayHD, double totalAmount, boolean isProcess) {
		String sql = "insert into Invoice values(?,?,?,?,?)";
		int i;
		try {
			Connection conn = DatabaseConnection.getConnection();
	...
	// tim 1 hoa don dua vao IdInvoice:
	public Invoice lookInvoiceALL(int idInvoice) {
		Invoice invoi = null;
		String sql = "SELECT userInfo.*, Invoice.id as idInvoice, Invoice.ngayHD, Invoice.isProcess FROM userInfo JOIN Invoice ON userInfo.id= Invoice.idUser where Invoice.id = ? ";
		ResultSet re = null;
		try {
			Connection conn = DatabaseConnection.getConnection();
			PreparedStatement pr = conn.prepareStatement(sql);
...
//**************chuc nang xu li hoa don
	...
	// lay Shoping cua 1 hoa don dua vao IdInvoice
	...
	// tu choi hoa don
	public boolean refuseAInvoice(int idInvoice) {
		String sql = "delete from Invoice where id=? and isProcess=0";
		int i;
		try {
			Connection conn = DatabaseConnection.getConnection();
			PreparedStatement pr = conn.prepareStatement(sql);
	...
	// truy van danh sach don hang dua vao ID USER
	public ArrayList<Invoice> getListInvoiceOfAUser(int idUser) {
		ArrayList<Invoice> result = new ArrayList<>();
		// lay danh sach hoa don cua idUser
		String sql = "SELECT userInfo.*, Invoice.id as idInvoice, Invoice.ngayHD,Invoice.totalAmount , Invoice.isProcess FROM userInfo JOIN Invoice ON userInfo.id= Invoice.idUser where idUser= ?";
		try {
			Connection conn = DatabaseConnection.getConnection();
			PreparedStatement pr = conn.prepareStatement(sql);
			pr.setInt(1, idUser);
			...
			while (re.next()) {
				Customer customer = new Customer(re.getInt("id"), re.getString("username"), re.getString("fullname"),
						re.getString("phone"), re.getString("address1"));
				int idInvoice = re.getInt("idInvoice");
				// goi lai cau lenh tim Cart cua id
				ShoppingCart shop = getShoppingCartInInvoiceID(idInvoice);
				Date date = re.getDate("ngayHD");
				java.util.Date utilDate = new java.util.Date(date.getTime());
				boolean isProcess = re.getBoolean("isProcess");
	...
	// su thong tin 1 hoa don
	public boolean edit(String date, int id) {
		String sql = "update Invoice set ngayHD = ? where id = ?";
		int i = 0;
		try {
			Connection conn = DatabaseConnection.getConnection();
			PreparedStatement pre = conn.prepareStatement(sql);
	...
	// su thong tin 1 hoa don
		public boolean editIDUser(int idUser) {
			String sql = "update Invoice set idUser = 0 where idUser = ?";
			int i = 0;
			try {
				Connection conn = DatabaseConnection.getConnection();
				PreparedStatement pre = conn.prepareStatement(sql);
