...
public class ControllerProduct extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (action.equals("remove")) {
			...
			sesion.setAttribute("listProductAdmin", listProduct);
			if (remove == true) {
				pw.print("Xóa thành công mã SP: " + id);
			} else {
				pw.print("Gặp lỗi mã SP: " + id);
			}
		}
		if (action.equals("removeview")) {
			...
			if (remove == true) {
				...
				pw.println("swal ( \"Thông báo\" ,  \"Xóa thành công thông tin sản phẩm!\" ,  \"success\" )");
				pw.println("});");
				pw.println("</script>");
				...
				pw.println("<script type=\"text/javascript\">");
				pw.println("	$(document).ready(function(){");
				pw.println("swal ( \"Lỗi\" ,  \"Không thể xóa SP!\" ,  \"error\" )");
				pw.println("});");
				pw.println("</script>");
			}
			// chuyen trang
			String url = "/admin/product/view.jsp";
			RequestDispatcher re = getServletContext().getRequestDispatcher(url);
		...
		if (action.equals("view")) {
			int id = Integer.parseInt(request.getParameter("id"));
			Product p = new ProductDAO().lookUp(id);
			request.setAttribute("product_View", p);
			String url = "/admin/product/view.jsp";
			RequestDispatcher re = getServletContext().getRequestDispatcher(url);
			re.forward(request, response);
		...
		if (action.equals("add")) {
			...
			// hien thong bao
			PrintWriter pw = response.getWriter();
			pw.println("Thêm thành công sản phẩm với ID: " + id);
//			pw.println(
//					"<script src=\"https://cdnjs.cloudflare.com/ajax/libs/limonte-sweetalert2/6.11.4/sweetalert2.all.js\"></script>");
//			pw.println("<script type=\"text/javascript\">");
//			pw.println("	$(document).ready(function(){");
//			pw.println("swal ( \"Thông báo\" ,  \"Thêm thành công SP!\" ,  \"success\" )");
//			pw.println("});");
//			pw.println("</script>");
//			// chuyen trang
//			Product p = new ProductDAO().lookUp(id);
...
//			re.include(request, response);
		...
		if (action.equals("edit")) {
			...
			if (edit == true) {
//				pw.println("Sửa thông tin sản phẩm " + id + "thành công");
				pw.println(
						"<script src=\"https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js\"></script>");
				pw.println(
						"<script src=\"https://cdnjs.cloudflare.com/ajax/libs/limonte-sweetalert2/6.11.4/sweetalert2.all.js\"></script>");
				pw.println("<script type=\"text/javascript\">");
				pw.println("	$(document).ready(function(){");
				pw.println("swal ( \"ThÃ´ng bÃ¡o\" ,  \"Cập nhật thành công thông tin sản phẩm!\" ,  \"success\" )");
				pw.println("});");
				pw.println("</script>");
				...
				pw.println("<script type=\"text/javascript\">");
				pw.println("	$(document).ready(function(){");
				pw.println("swal ( \"ThÃ´ng bÃ¡o\" ,  \"Không thể thay đổi thông tin sản phẩm!\" ,  \"error\" )");
				pw.println("});");
				pw.println("</script>");
			}
			// chuyen trang
		...
		if (action.equals("lookUp")) {
			...
			request.setAttribute("numberSearch", "TÃ¬m Ä‘Æ°á»£c " + listSearch.size() + " sáº£n pháº©m liÃªn quan");
