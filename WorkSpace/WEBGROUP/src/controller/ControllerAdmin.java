	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (action.equals("login")) {
			...
			if (user != null) {
				if (pass.equals(user.getPass())) {// mhap dung pass
					...
					HttpSession session = request.getSession();
					// anh xa userInfo
					Customer cu = new CustomerDAO().lookUpID(user.getIdUser());
		...
		if (action.equals("displayInvoiceProcess")) {
			...
			if (unprocessedInvoices == null || unprocessedInvoices.size() <= 0) {
				...
				pw.println("<script type=\"text/javascript\">");
				pw.println("	$(document).ready(function(){");
				pw.println("swal ( \"Thông báo\" ,  \"Không có hóa đơn nào chờ xử lí!\" ,  \"info\" )");
				pw.println("});");
				pw.println("</script>");
				...
				RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/admin/unprocess_invoice/invoices.jsp");
		...
		if (action.equals("processInvoice")) {
			// b1: cập nhật lại muc isProcess trong bang Invoice DB
			// b2: chuyen trang
			HttpSession session = request.getSession();
...
//			int id = Integer.parseInt(request.getParameter("id"));
		...
		if (action.equals("removeInvoice")) {
			...
			// hien thong bao
			request.setAttribute("removeInvoiceDone", "Xóa thanh công hóa đơn" + idInvoice);
