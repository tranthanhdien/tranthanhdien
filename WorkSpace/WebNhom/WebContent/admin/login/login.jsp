			...
			<div class="modal-header">
				<%
					if (request.getAttribute("NotAdmin") != null) {
						out.print("<font color=\"red\">" + (String) request.getAttribute("NotAdmin") + "</font");
					}
				%>
			...
			<div class="modal-body">
				<form action="ControllerAdmin?action=login" method="post"
					...
					<p class="text-center dont-do">
						Don't have an account? <a href="register.jsp" data-toggle="modal"
							class="text-dark"> Register Now</a>
					</p>
					<a>Các User có thể đăng nhập, hãy test tất cả trường hợp</a>
	<%for(Login l: new LoginDAO().getListLogin()){ %>
	<p><%=l.isAdmin() %></p>
	<%} %>
				</form>
			</div>
