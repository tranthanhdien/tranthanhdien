...
//		// loc theo type SP
		...
		if (action.equals("login")) {
			...
			if (user != null) {
				if (pass.equals(user.getPass())) {// mhap dung pass
					...
					request.setAttribute("pass_err", "Password khÃ´ng Ä‘Ãºng, nháº­p láº¡i");
					getServletContext().getRequestDispatcher("/login.jsp").forward(request, response);
				}
		...
		if (action.equals("completeOrder")) {
			...
			if (cus.getUsername().equals("no name") || cus.getFullName().equals("no fullname")
				...
				String form = "dangvanda.itnlu@gmail.com";
				...
				MailUtil.sendMail(to, subject, "hello");
				...
				pw.println("swal ( \"Thông bán\" ,  \"Đặt hàng thành công. Cảm ơn quý khách\" ,  \"success\" )");
				pw.println("});");
				pw.println("</script>");
