						...
						<table id="example" class="table table-hover">
							...
							<tbody>
								...
								<tr>
									...
									<td><%=c.getL().getIdUser()%></td>
									<td><%=c.getL().getEmail()%></td>
									<td><%=c.getC().getUsername()%></td>
									<td><%=c.getC().getFullName()%></td>
									<td><%=c.getC().getPhone()%></td>
									<td><%=c.getC().getAddress()%></td>
									<td>
										<table>
											<tr>
												<td>
													<form
														action="<%=request.getContextPath()%>/ControllerUser?action=remove&id=<%=c.getL().getIdUser()%>"
														onsubmit="return submitForm(this);" method="post">
														<button type="submit" class="btn btn-danger btn-sm"
															style="font-size: 13px;"
															onclick="if (!confirm('bạn có chắc chắn xóa SP: ' + <%=c.getC().getId()%> +'?')) { return false }">
															<span class="fa fa-trash-o"></span>
														</button>
													</form>
												</td>
												<td><a
													href="<%=request.getContextPath()%>/ControllerUser?action=lookToEdit&id=<%=c.getC().getId()%>"
													class="btn btn-primary btn-sm" style="font-size: 13px;"><span
														class="fa fa-edit"></span></a></td>
												<td><a
													href="<%=request.getContextPath()%>/ControllerUser?action=lookUp&id=<%=c.getC().getId()%>"
					...
					<div class="modal-body">
						<table style="width: 100%;">
							<tr>
								<td><div class="input-group" style="display: grid;">
										<label for="ma"><b>Mã người dùng</b></label> <input
											type="text" value="<%=new CustomerDAO().dynamicIDCustomer() %>" name="id">
									</div></td>
								<td>
							...
							<td colspan="3"><b>Admin </b>
										<input type="radio" name="admin" value="yes">Có
										<input type="radio" name="admin" value="no">Không
									</td>
							</tr>
							<tr>
		...
		<div id='printarea'>
			<table>
				...
				<%
					count = 1;
					for (User user : list) {
						Customer c = user.getC();
				%>
				<tr>
					<th><%=count%></th>
					<td style="text-align: center"><%=c.getId()%></td>
					<td style="text-align: center"><%=c.getUsername()%></td>
					<td style="text-align: center"><%=c.getFullName()%></td>
					<td style="text-align: center"><%=c.getPhone()%></td>
					<td style="text-align: center"><%=c.getAddress()%></td>
				</tr>
				<%
					count++;
					}
